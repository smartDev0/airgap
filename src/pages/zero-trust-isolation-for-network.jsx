...
const ZeroTrustIsolationNetworkPage = ({ path }) => {
    const data = useStaticQuery(graphql`
        query {
            product: pagesYaml(id: {eq: "zti-network"}) {
                hero {
                    title
                }
                copy {
    ...
    return (
        <>
            <SEO
                title="Zero Trust Isolation for Network"
                description="Airgap’s Agentless Zero Trust Isolation for Network (SaaS) platform can be deployed within minutes. Airgap ensures enterprise assets are protected against ransomware threats and propagation."
                url={path}
            />
            ...
            <div style={{ backgroundColor: '#F3F6F8' }}>
                <Container fluid="xl" className="pt-10">
                    <Row className="pb-10 justify-content-center align-items-center">
                        <Col lg={6}>
                            <CustomMarkdown>{product.copy.text}</CustomMarkdown>
                        </Col>
                        <Col lg={6} className="mt-4 mt-lg-0">
                            <Image
                                alt="modus operandi of Zero Trust Isolation"
                                fluid={product.copy.image.childImageSharp.fluid}
                            />
                        </Col>
                    </Row>
                </Container>
...
export default ZeroTrustIsolationNetworkPage;
